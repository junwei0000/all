...
import com.longcheng.lifecareplan.modular.helpwith.connonEngineering.adapter.ConnonListAdapter;
import com.longcheng.lifecareplan.modular.helpwith.connonEngineering.bean.ConnonAfterBean;
import com.longcheng.lifecareplan.modular.helpwith.connonEngineering.bean.ConnonListDataBean;
import com.longcheng.lifecareplan.modular.helpwith.connonEngineering.bean.ConnonItemBean;
 ...
 * 康农首页列表
 */
public class ConnonListActivity extends BaseListActivity<ConnonContract.View, ConnonPresenterImp<ConnonContract.View>> implements ConnonContract.View {
    ...
    List<ConnonItemBean> helpAllList = new ArrayList<>();
    ...
    private void getList(int page) {
        ...
        mPresent.setListViewData(user_id,
                timexu, Search, progressxu, status, help_status, goods_id, page, pageSize);
    }


    @Override
    protected ConnonPresenterImp<ConnonContract.View> createPresent() {
        return new ConnonPresenterImp<>(mContext, this);
    }
    ...
    @Override
    public void ListSuccess(ConnonListDataBean responseBean, int backPage) {
        ListUtils.getInstance().RefreshCompleteL(helpListview);
        String status_ = responseBean.getStatus();
        if (status_.equals("400")) {
            ToastUtils.showToast(responseBean.getMsg());
        } else if (status_.equals("200")) {
            ConnonAfterBean mEnergyAfterBean = responseBean.getData();
            if (mEnergyAfterBean != null) {
                ...
                List<ConnonItemBean> helpList = mEnergyAfterBean.getHelp_msg();
                ...
                if (mHomeHotPushAdapter == null) {
                    mHomeHotPushAdapter = new ConnonListAdapter(mContext, helpList);
    ...
    ConnonListAdapter mHomeHotPushAdapter;
    ...
    private void back() {
        if (!TextUtils.isEmpty(skiptype) && (skiptype.equals("MyGratitudeActivity")
