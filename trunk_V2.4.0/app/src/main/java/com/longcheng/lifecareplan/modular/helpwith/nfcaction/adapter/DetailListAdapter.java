...
public class DetailListAdapter extends BaseAdapterHelper<NFCDetailItemBean> {
    private final ImageLoader asyncImageLoader;
    ViewHolder mHolder = null;
    Activity context;

    public DetailListAdapter(Activity context, ArrayList<NFCDetailItemBean> list) {
        super(context, list);
    ...
    @Override
    public View getItemView(int position, View convertView, ViewGroup parent, List<NFCDetailItemBean> list, LayoutInflater inflater) {
        if (convertView == null) {
            convertView = inflater.inflate(R.layout.home_bangdan_bless_item2, parent, false);
            mHolder = new ViewHolder(convertView);
            convertView.setTag(mHolder);
        } else {
            mHolder = (ViewHolder) convertView.getTag();
        }
        NFCDetailItemBean mRankItemBean = list.get(position);
        ...
        String jieqi = mRankItemBean.getUser_identity_jieqi();
        if (!TextUtils.isEmpty(jieqi)) {
            mHolder.item_tv_jieeqi.setText(jieqi);
            mHolder.item_tv_jieeqi.setVisibility(View.VISIBLE);
        } else {
            mHolder.item_tv_jieeqi.setVisibility(View.GONE);
        }
        mHolder.item_tv_rank.setText(mRankItemBean.getPrice());
        List<String> identity_img = mRankItemBean.getUser_identity_img();
        ...
        mHolder.item_tv_typetitle.setText("为您送出能量");
        return convertView;
    }
