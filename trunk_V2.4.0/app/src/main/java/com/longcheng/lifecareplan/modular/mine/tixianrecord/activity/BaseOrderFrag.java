...
import com.longcheng.lifecareplan.modular.mine.tixianrecord.adapter.TXRecordAdapter;
import com.longcheng.lifecareplan.modular.mine.tixianrecord.bean.TXRecordAfterBean;
...
public abstract class BaseOrderFrag extends BaseListFrag<TXRecorrdContract.View, TXRecorrdPresenterImp<TXRecorrdContract.View>> implements TXRecorrdContract.View {
    ...
    List<TXRecordItemBean> mAllList = new ArrayList<>();
    private TXRecordAdapter mAdapter;


    public static final int INDEX_ALL = 0;
    public static final int INDEX_MATCH = 1;
    public static final int INDEX_COMING = 2;
    ...
    @Override
    protected TXRecorrdPresenterImp<TXRecorrdContract.View> createPresent() {
        return new TXRecorrdPresenterImp<>(this);
    }
    ...
    @Override
    public void ListSuccess(TXRecordDataBean responseBean, int back_page) {
        ListUtils.getInstance().RefreshCompleteL(dateListview);
        String status_ = responseBean.getStatus();
        if (status_.equals("400")) {
            ToastUtils.showToast(responseBean.getMsg());
        } else if (status_.equals("200")) {
            TXRecordAfterBean mOrderAfterBean = responseBean.getData();
            if (mOrderAfterBean != null) {
                List<TXRecordItemBean> mList = mOrderAfterBean.getList();
                ...
                if (mAdapter == null) {
                    mAdapter = new TXRecordAdapter(getActivity(), mList, mHandler, getType());
    ...
    @Override
    public void editSuccess(EditListDataBean responseBean) {
        String status = responseBean.getStatus();
        if (status.equals("400")) {
            ToastUtils.showToast(responseBean.getMsg());
        } else if (status.equals("200")) {
            ToastUtils.showToast(responseBean.getMsg());
            ((TiXianRecordListActivity) getActivity()).reLoadList();
        }
    }
    ...
    Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 1:
                    String user_zhufubao_order_id = (String) msg.obj;
                    mPresent.agreeePairsU(user_id, user_zhufubao_order_id);
                    break;
            }
        }
    };
