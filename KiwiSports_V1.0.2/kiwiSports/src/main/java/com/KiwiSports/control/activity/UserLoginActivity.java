...
public class UserLoginActivity extends BaseActivity {
		...
		case R.id.wxlogin:
			if (!wxHandler.isClientInstalled()) {
				CommonUtils.getInstance().initToast(
						getString(R.string.wxlogin_tishi));
			} else {
	...
	private void addWXPlatform() {
		...
		String appId = Constants.WEIXIN_APP_ID;
		String appSecret = Constants.WEIXIN_APP_SECRET;
		// 添加微信平台
	...
	/**
	 * 授权。如果授权成功，则获取用户信息 ------------value------------Bundle[{access_token=
	 * 4_mpMe-lYShZIC-BAOC_RXaSErr0QlqRXpOxWObAfY3zgbbp7gw5jNeNbri247QPRqT-nHW5eazu5fm7vkx0wS6nTXnlhxchlxnIcx3kxy
	 * b B U , refresh_token=
	 * 4_OlgBcOQy03lNpVO2wD7-qD8zReHTIjObUlcb0-rxSS_gGySjn4cosMSIgRdupmRK2LgQS0oqXZmQ6k07VcXZbKAKhaCKsMwt6ZrnWNNc
	 * Y 0 s , openid=o33YMuAiLhY5ggJuG05rk0Xai1sg, expires_in=7200,
	...
	private void login(SHARE_MEDIA platform) {
		showDilag();
		mController.doOauthVerify(UserLoginActivity.this, platform,
				new UMAuthListener() {
					@Override
	...
	/**
	 * 获取用户信息 ------------getUserInfo------------{unionid=
	 * oxPIquDANxen523S4ZlIyACnk0nM, country=中国, nickname=段军伟, city=海淀,
	 * language=zh_CN, province=北京, headimgurl=http://wx.qlogo.cn/mmopen/vi_32/
	 * Q0j4TwGTfTJJialkDlK6EenxibchE80J4e1PMUfGYVg9uO9QUU4ALJVvga83QEDXxt0I5FrFdPKA6MrJTMu34Okw
	 * /0, sex=1, openid=o33YMuAiLhY5ggJuG05rk0Xai1sg}
	 * 
	 * @param platform
	 */
	private void getUserInfo(final SHARE_MEDIA platform) {
		mController.getPlatformInfo(UserLoginActivity.this, platform,
				new UMDataListener() {
					...
					public void onComplete(int status, Map<String, Object> info) {
						if (status == StatusCode.ST_CODE_SUCCESSED
							...
							if (platform.equals(SHARE_MEDIA.WEIXIN)) {
								...
								int sex_ = (Integer) info.get("sex");
								sex=String.valueOf(sex_);
	...
	private void showLoginFailureInfo() {
		CommonUtils.getInstance().initToast(
				getString(R.string.wxlogin_tishifali));
		CommonUtils.getInstance().setOnDismissDialog(mDialog);
	}
	...
	private void saveUserInfo(UserLoginInfo loginInfo) {
		...
		bestDoInfoEditor.putString("sexNew", sex+"");
