...
public class TestSkyActivity extends BaseActivity {
	...
	/**
	 * lapCount =6; minAltidue=114; maxAltitude=206 _nMaxSlopeAngle=13
	 * upHillDistance=1776 verticalDistance=92
	 */
	@Override
...
//					"124.txt");
	...
	public ArrayList<MainLocationItemInfo> parseJSON(JSONObject jsonObject) {
		try {
			...
			for (int i = 0; i < Ob.length(); i++) {
				...
				MainLocationItemInfo mMainSportInfo = new MainLocationItemInfo(
						latitude, longitude, speed, altitude, accuracy,
						nStatus, nLapPoint, nLapTime,nLapTime, duration, distance,
						latitudeOffset, longitudeOffset,mLatLng);
	...
	private void setStatusDownWithDistance() {
		...
		int skiDis = (int) (allpointList.get(allpointList.size() - 1)
				.getDistance()
				- allpointList.get(nHighestPointIndex).getDistance() + temdistance);
