...
public class PullDownListView extends FrameLayout implements
	...
	private void addRefreshBar() {
		...
		// 刷新头部的view
		view = LayoutInflater.from(getContext()).inflate(R.layout.refresh_top,
				null);
		...
		mUpdateContent = (FrameLayout) getChildAt(0).findViewById(
				R.id.refreshtop_iv_content);
		re_img = (ImageView) getChildAt(0).findViewById(R.id.refresh_img);
		try {
			AnimationDrawable frameAnimation = null;
			if (frameAnimation == null) {
				frameAnimation = (AnimationDrawable) context.getResources()
						.getDrawable(R.drawable.xiala_anim);
			}
		...
		mProgressBar.setIndeterminateDrawable(getResources().getDrawable(
				R.drawable.dialog_style_xml_icon));
	...
	protected void onFinishInflate() {
		...
		footer_layout = (LinearLayout) LayoutInflater.from(getContext())
				.inflate(R.layout.refresh_lay, null);
		foot = (LinearLayout) LayoutInflater.from(getContext()).inflate(
				R.layout.refresh_foot, null);
		more = (TextView) foot.findViewById(R.id.refreshbottom_more);
		refreshbottom_morebottom = (TextView) foot
				.findViewById(R.id.refreshbottom_morebottom);
		mProgressBar2 = (ProgressBar) foot
				.findViewById(R.id.refreshbottom_refbar);
		mProgressBar2.setIndeterminate(false);
		mProgressBar2.setIndeterminateDrawable(getResources().getDrawable(
				R.drawable.dialog_style_xml_icon));
		mProgressBar2.setVisibility(View.GONE);
		if (useempty) {
			header = (LinearLayout) LayoutInflater.from(getContext()).inflate(
					R.layout.refresh_lay, null);
		...
		mListView.setOnScrollListener(new OnScrollListener() {
			...
			public void onScrollStateChanged(AbsListView view, int scrollState) {
				...
				int t = view.getCount();
				System.err.println("onScrollStateChanged== " + f + "  " + lastv
						+ "  " + t + " ");
				/**
				...
				System.err.println(LOADMORESTATUS + " " + isEnd + " "
						+ MOVELOAD + " " + hasMore + " " + isAutoLoadMore);
				// 1.没有正在加载更多；2.到底部；3.是否是滑动来执行加载操作； 4.当屏幕停止滚动时;5.可以加载更多；6.自动加载更多
				if (!LOADMORESTATUS && isEnd && MOVELOAD
						&& scrollState == SCROLL_STATE_IDLE && hasMore
						&& isAutoLoadMore) {
					onLoadMore();
				}
			}
		...
		if (loadstatus.equals("over")) {
			...
			if (TextUtils.isEmpty(skipfrom)) {
				mListView.smoothScrollBy(
						-footer_layout.getMeasuredHeight() - 5, 200);
			} else {
				mListView.smoothScrollBy(-footer_layout.getMeasuredHeight()
						+ refreshbottom_morebottom.getMeasuredHeight() + 25, 200);
			}
			// 注释---：每次都初始化
			loadstatus = "";
			isEnd = false;
