...
public class SliderRangeBar extends View {
	...
	private int DEFAULT_TICK_COUNT = 3;
	private float DEFAULT_TICK_HEIGHT_DP = 10;// 默认刻度的高度
	private float DEFAULT_CONNECTING_LINE_WEIGHT_PX = -3;// x轴默认所在位置
	private int DEFAULT_THUMB_IMAGE_NORMAL = R.drawable.userset_slider_img;
	private int DEFAULT_THUMB_IMAGE_PRESSED = R.drawable.userset_slider_img;
	...
	private SliderRangeBar.OnRangeBarChangeListener mListener;
	private int mLeftIndex = 0;
	private int mRightIndex = mTickCount - 1;
	/**
	 * 默认位置
	 */
	private int mMoRenIndex = 1;
	...
	private void getPhonePixelsView(Context context) {
		...
		mBarColor = context.getResources().getColor(R.color.contents_color);
		mBarWeight = 3;
		mMoRenIndex=TextSizeUtils.getInstance().NOWTEXTSIZE;
	}
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		 ...
		 * 2.设置初始位置
		 */
		mRightThumb.setX(marginLeft + ((float) mMoRenIndex / mRightIndex)
				* barLength);
	...
	public void setOnRangeBarChangeListener(
			SliderRangeBar.OnRangeBarChangeListener listener) {
		mListener = listener;
	}
	...
	private void rangeBarInit(Context context, AttributeSet attrs) {
		...
		try {
			...
			// attributes.
			Integer tickCount = ta.getInteger(R.styleable.RangeBar_tickCount,
					DEFAULT_TICK_COUNT);
