package com.zh.education.control.datepicker;
...
public class MonthDateView extends View {
	...
	private int mDayColor = Color.parseColor("#797a7e");
	private int mSelectDayColor = Color.parseColor("#ffffff");
	private int mSelectBGColor = Color.parseColor("#097fe1");
	...
	protected void onDraw(Canvas canvas) {
		...
		String dayString;
		int mMonthDays = DatesUtils.getInstance().getMonthDays(mSelYear, mSelMonth);
		int weekNumber = DatesUtils.getInstance().getFirstDayWeek(mSelYear, mSelMonth);
		...
		for(int day = 0;day < mMonthDays;day++){
			...
			if(dayString.equals(mSelDay+"")){
				...
				float circley = (float) (mRowSize * row+(float)mRowSize/2);
				canvas.drawCircle(circleX, circley, mColumnSize/4, mPaint);
				//记录第几行，即第几周
				weekRow = row + 1;
			}
			//绘制事务圆形标志
			...
			drawCircle(row,column,day + 1,canvas,mSelDay);
	...
	private void drawCircle(int row,int column,int day,Canvas canvas,int mSelDay){
		if(daysHasThingList != null && daysHasThingList.size() >0){
			if(!daysHasThingList.contains(day))
				return;
//			String dayss=day+"";
//			if(dayss.equals(mSelDay+"")){
				mPaint.setColor(mCircleColor);
				float circleX = (float) (mColumnSize * column+mColumnSize*0.5 );
				float circley = (float) (mRowSize * row +mColumnSize*0.65);
				canvas.drawCircle(circleX, circley, mCircleRadius, mPaint);
//			}else{
//				Paint mPaints=new Paint();
//				mPaints.setStyle(Paint.Style.STROKE);
//				mPaints.setColor(mCircleColor);
//				float circleX = (float) (mColumnSize * column+mColumnSize*0.5 );
//				float circley = (float) (mRowSize * row +mColumnSize*0.65);
//				canvas.drawCircle(circleX, circley, mCircleRadius, mPaints);
//			}
		}
	}
	@Override
	...
	public void onLeftClick(){
		...
		if(month == 0){//若果是1月份，则变成12月份
			year = mSelYear-1;
			month = 11;
		}else if(DatesUtils.getInstance().getMonthDays(year, month) == day){
			//如果当前日期为该月最后一点，当向前推的时候，就需要改变选中的日期
			month = month-1;
			day = DatesUtils.getInstance().getMonthDays(year, month);
		}else{
			month = month-1;
		}
