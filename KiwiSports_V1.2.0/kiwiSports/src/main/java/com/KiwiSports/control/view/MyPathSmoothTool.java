...
/**
 * 轨迹优化工具类 Created by my94493 on 2017/3/31.
 * <p>
 * 使用方法：
 * <p>
 * PathSmoothTool pathSmoothTool = new PathSmoothTool();
 * pathSmoothTool.setIntensity(2);//设置滤波强度，默认3 List<LatLng> mList =
 * LatpathSmoothTool.kalmanFilterPath(list);
 */

public class MyPathSmoothTool {
	private int mIntensity = 3;
	private float mThreshhold = 0.3f;
	private float mNoiseThreshhold = 10;

	public MyPathSmoothTool() {
	 ...
	 * 
	 * @param originlist
	 *            原始轨迹list,list.size大于2
	 * @return 优化后轨迹list
	 */
	public List<MainLocationItemInfo> pathOptimize(
			List<MainLocationItemInfo> originlist) {

		List<MainLocationItemInfo> list = removeNoisePoint(originlist);// 去噪
		List<MainLocationItemInfo> afterList = kalmanFilterPath(list,
				mIntensity);// 滤波
		List<MainLocationItemInfo> pathoptimizeList = reducerVerticalThreshold(
				afterList, mThreshhold);// 抽稀
	 ...
	 * @return 滤波处理后的轨迹list
	 */
	public List<MainLocationItemInfo> kalmanFilterPath(
			List<MainLocationItemInfo> originlist) {
	 ...
	 * @return
	 */
	public List<MainLocationItemInfo> removeNoisePoint(
			List<MainLocationItemInfo> originlist) {
	 ...
	 * @return 抽稀后的轨迹list
	 */
	public List<MainLocationItemInfo> reducerVerticalThreshold(
			List<MainLocationItemInfo> inPoints) {
	 ...
	 * @return
	 */
	private List<MainLocationItemInfo> kalmanFilterPath(
			List<MainLocationItemInfo> originlist, int intensity) {
		List<MainLocationItemInfo> kalmanFilterList = new ArrayList<MainLocationItemInfo>();
		if (originlist == null || originlist.size() <= 2)
			return originlist;
		initial();// 初始化滤波参数
		LatLng latLng = null;
		MainLocationItemInfo lastLoc = originlist.get(0);
		kalmanFilterList.add(lastLoc);
		for (int i = 1; i < originlist.size(); i++) {
			MainLocationItemInfo curLoc = originlist.get(i);
			latLng = kalmanFilterPoint(lastLoc.getmLatLng(),
					curLoc.getmLatLng(), intensity);
			if (latLng != null) {
				curLoc.setmLatLng(latLng);
				kalmanFilterList.add(curLoc);
				lastLoc = curLoc;
			} else {
				String latLngDashedStatus = curLoc.getnLapTime();
	 ...
	 * @return 滤波后本次定位点坐标值
	 */
	private LatLng kalmanFilterPoint(LatLng lastLoc, LatLng curLoc,
			int intensity) {
	...
	private LatLng kalmanFilter(double oldValue_x, double value_x,
			double oldValue_y, double value_y) {
		lastLocation_x = oldValue_x;
		currentLocation_x = value_x;
		gauss_x = Math.sqrt(pdelt_x * pdelt_x + mdelt_x * mdelt_x) + m_Q; // 计算高斯噪音偏差
		kalmanGain_x = Math.sqrt((gauss_x * gauss_x)
				/ (gauss_x * gauss_x + pdelt_x * pdelt_x))
				+ m_R; // 计算卡尔曼增益
		...
		kalmanGain_y = Math.sqrt((gauss_y * gauss_y)
				/ (gauss_y * gauss_y + pdelt_y * pdelt_y))
				+ m_R; // 计算卡尔曼增益
	...
	/*************************** 抽稀算法 *************************************/
	private List<MainLocationItemInfo> reducerVerticalThreshold(
			List<MainLocationItemInfo> inPoints, float threshHold) {
		if (inPoints == null) {
			return null;
		}
		if (inPoints.size() <= 2) {
		...
		List<MainLocationItemInfo> ret = new ArrayList<MainLocationItemInfo>();
		for (int i = 0; i < inPoints.size(); i++) {
			MainLocationItemInfo pre = getLastLocation(ret);
			MainLocationItemInfo cur = inPoints.get(i);

			if (cur.getnLapTime().equals("latLngDashedStatus_true")) {
				ret.add(cur);
			} else {
				...
				MainLocationItemInfo next = inPoints.get(i + 1);
				double distance = calculateDistanceFromPoint(cur.getmLatLng(),
						pre.getmLatLng(), next.getmLatLng());
	...
	private static MainLocationItemInfo getLastLocation(
			List<MainLocationItemInfo> oneGraspList) {
		if (oneGraspList == null || oneGraspList.size() == 0) {
			return null;
		}
		int locListSize = oneGraspList.size();
		MainLocationItemInfo lastLocation = oneGraspList.get(locListSize - 1);
		return lastLocation;
	}
	 ...
	 * 
	 */
	private static double calculateDistanceFromPoint(LatLng p,
			LatLng lineBegin, LatLng lineEnd) {
	...
	private List<MainLocationItemInfo> reduceNoisePoint(
			List<MainLocationItemInfo> inPoints, float threshHold) {
		if (inPoints == null) {
			return null;
		}
