	...
	public HashMap<String, Object> parseJSON(JSONObject jsonObject) {
		...
		try {
			...
			if (status.equals("200")) {
				...
				String sex = jsonOb.optString("sex", Constants.getInstance().SEX_MALE);
				String token = jsonOb.optString("token", "");
				String access_token = jsonOb.optString("access_token", "");
				UserLoginInfo loginInfo = new UserLoginInfo(uid, nick_name,
						album_url, hobby, sex, token, access_token);
