 ...
 * @Description 类描述：活动管理-已报名
 */
public class CampaignManagerRegisteredFragment extends Fragment implements OnRefreshListioner {
	...
	public CampaignManagerRegisteredFragment() {
		super();
	}

	@SuppressLint("ValidFragment")
	public CampaignManagerRegisteredFragment(Activity mContext, String uid) {
		super();
	...
	private void getData() {
		if (isAttached && mContext != null && mContext.selectArg0 == Integer
				.valueOf(mContext.getResources().getString(R.string.order_index_waitpay))) {
	...
	private void updateList() {
		...
		if (mAdapter == null) {
			mAdapter = new CampaignManagerListAdapter(mContext, mList, "Registered");
	...
	Handler mPullDownViewHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case DATAUPDATEOVER:
				...
				nideBottom();
				refreshOrLoadMoreLoading = false;
				break;
			case REFLESH:
				boolean ff = refreshOrLoadMoreLoading;
				if (!ff) {
					refreshOrLoadMoreLoading = true;
					init();
					getData();
				}
				break;
