...
public class PriceRangeBar extends View {
	...
	private String TAG = "RangeBar";

	// Default values for variables
	private int DEFAULT_TICK_COUNT = 6;
	private float DEFAULT_TICK_HEIGHT_DP = 24;
	private float DEFAULT_CONNECTING_LINE_WEIGHT_PX = 3;
	private int DEFAULT_THUMB_IMAGE_NORMAL = R.drawable.stadiummaplist_bottom_pricethumb;
	private int DEFAULT_THUMB_IMAGE_PRESSED = R.drawable.stadiummaplist_bottom_pricethumb;

	// Corresponds to android.R.color.holo_blue_light.

	// Indicator value tells Thumb.java whether it should draw the circle or not
	private float DEFAULT_THUMB_RADIUS_DP = -1;
	private int DEFAULT_THUMB_COLOR_NORMAL = -1;
	private int DEFAULT_THUMB_COLOR_PRESSED = -1;
	...
	private PriceRangeBar.OnRangeBarChangeListener mListener;
	...
	public PriceRangeBar(Context context) {
		super(context);
		getPhonePixelsView(context);
	}

	public PriceRangeBar(Context context, AttributeSet attrs) {
		super(context, attrs);
		getPhonePixelsView(context);
		rangeBarInit(context, attrs);
	}

	public PriceRangeBar(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		getPhonePixelsView(context);
		rangeBarInit(context, attrs);
	}

	private void getPhonePixelsView(Context context) {
		mConnectingLineColor = context.getResources().getColor(R.color.blue);
		mBarColor = context.getResources().getColor(R.color.text_noclick_color);
		int windth = ConfigUtils.getInstance().getPhoneWidHeigth(context).widthPixels;
		mDefaultWidth = windth;
		mBarWeight = 4;
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		float marginLeft = mLeftThumb.getHalfWidth();
		float barLength = w - 2 * marginLeft;
		mBar = new Bar(ctx, marginLeft, yPos, barLength, mTickCount, 0, mTickCount, mConnectingLineColor, mTickHeightDP,
	 ...
	 *            existing listener
	 */
	public void setOnRangeBarChangeListener(PriceRangeBar.OnRangeBarChangeListener listener) {
		mListener = listener;
	}
	...
	public void setTickCount(int tickCount) {
		...
		if (isValidTickCount(tickCount)) {
			...
			createBar(0, mTickCount - 1);
			createThumbs(mLeftIndex, mRightIndex);
		} else {
	...
	public void setThumbIndices(int leftThumbIndex, int rightThumbIndex) {
		if (indexOutOfRange(leftThumbIndex, rightThumbIndex)) {
			...
			createThumbs(leftThumbIndex, rightThumbIndex);
	...
	private void rangeBarInit(Context context, AttributeSet attrs) {
		...
		try {
			...
			mBarWeight = ta.getDimension(R.styleable.RangeBar_barWeight, mBarWeight);
			mBarColor = ta.getColor(R.styleable.RangeBar_barColor, mBarColor);
			mConnectingLineWeight = ta.getDimension(R.styleable.RangeBar_connectingLineWeight,
					DEFAULT_CONNECTING_LINE_WEIGHT_PX);
			mConnectingLineColor = ta.getColor(R.styleable.RangeBar_connectingLineColor, mConnectingLineColor);
	 ...
	 * @param none
	 */
	public void createBar(int leftThumbIndex, int rightThumbIndex) {

		mBar = new Bar(getContext(), getMarginLeft(), getYPos(), getBarLength(), mTickCount, leftThumbIndex,
				rightThumbIndex, mConnectingLineColor, mTickHeightDP, mBarWeight, mBarColor);
		invalidate();
	}
	 ...
	 * @param none
	 */
	public void createThumbs(int mLeftIndex, int mRightIndex) {
	...
	public interface OnRangeBarChangeListener {
		...
		public void onIndexChangeListener(PriceRangeBar rangeBar, int leftThumbIndex, int rightThumbIndex);
	}
}
