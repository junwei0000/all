...
public class ColorArcProgressBar extends View {
	...
	private OnWalkPointClick onPointClick;
	...
	private float curValuesShowIfLarge = curValues;
	...
	private String bgArcColor = "#42ABFF";
	...
	public interface OnWalkPointClick {
		void onClick();
	}

	public OnWalkPointClick getOnPointClick() {
		return onPointClick;
	}

	public void setOnPointClick(OnWalkPointClick onPointClick) {
	...
	private void initListener() {
		setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				float x = event.getX();
				float y = event.getY();
				float textX = centerX, textY = centerY - 1.5f * textSize / 3;
				float dx = x - textX, dy = y - textY;
	...
	private void initCofig(Context context, AttributeSet attrs) {
		...
		titleString1 = a.getString(R.styleable.ColorArcProgressBar_string_title1);
		titleString2 = a.getString(R.styleable.ColorArcProgressBar_string_title2);
	...
	private void initView() {
		...
		diameter = 55 * DpUtil.getScreenWidth(getContext()) / 100;
		// 弧形的矩阵区域
		bgRect = new RectF();
		...
		vTextPaint.setTypeface(CommonUtils.getAssetTypeFace(getContext()));
	...
	protected void onDraw(Canvas canvas) {
			...
			for (int i = 0; i < 40; i++) {
				...
				if (i % 5 == 0) {
					...
					canvas.drawLine(centerX, centerY - diameter / 2 - progressWidth / 2 - DEGREE_PROGRESS_DISTANCE,
							centerX, centerY - diameter / 2 - progressWidth / 2 - DEGREE_PROGRESS_DISTANCE - longdegree,
							degreePaint);
				} else {
		...
		if (isNeedTitle) {
			canvas.drawText(titleString1, centerX, centerY - 3 * textSize / 3, curSpeedPaint);
			curSpeedPaint.setTextSize(dipToPx(13));
			canvas.drawText(titleString2, centerX, centerY - 1.5f * textSize / 3, curSpeedPaint);
		}
		if (isNeedContent) {
			/**
			 * 显示步数数据不动
			 */
			canvas.drawText(String.format("%.0f", curValuesShowIfLarge), centerX, centerY + 2 * textSize / 3, vTextPaint);
//			canvas.drawText(String.format("%.0f", curValues), centerX, centerY + 2 * textSize / 3, vTextPaint);
		}
		if (isNeedUnit) {
			canvas.drawText(hintString, centerX, centerY + 3.5f * textSize / 3, hintPaint);
		}
	...
	public void setCurrentValues(float currentValues) {
		curValuesShowIfLarge = currentValues;
	...
	private void setAnimation(float last, float current, int length) {
		...
		progressAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
			...
			public void onAnimationUpdate(ValueAnimator animation) {
				...
				float k2 = curValuesShowIfLarge > maxValues ? curValuesShowIfLarge / maxValues : 1;
				curValues = currentAngle * k2 / k;
			}
