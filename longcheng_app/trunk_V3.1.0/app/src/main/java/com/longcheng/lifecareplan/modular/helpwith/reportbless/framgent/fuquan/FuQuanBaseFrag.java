...
public abstract class FuQuanBaseFrag extends BaseListFrag<ReportContract.View, ReportPresenterImp<ReportContract.View>> implements ReportContract.View {
        ...
        dateListview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (mallList != null && mallList.size() > 0 && position <= mallList.size()) {
                    Intent intent = new Intent(mActivity, FuQRepListActivity.class);
                    intent.setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
                    if (getType() == 0) {
                        intent.putExtra("type", 3);
                        intent.putExtra("keyword", mallList.get(position - 1).getShow_en());
                        intent.putExtra("keywordname", mallList.get(position - 1).getShow_name());
                    } else {
                        String date = DatesUtils.getInstance().getDateGeShi(mallList.get(position - 1).getShow_name(), "yyyy年MM月dd日", "yyyy-MM-dd");
    ...
    List<ReportItemBean> mallList = new ArrayList<>();
