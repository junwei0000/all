 ...
 * 24节气创业中心---续费银行卡
 */
public class PioneerXuFeiNextActivity extends BaseActivity {
    ...
    public void initDataAfter() {
        entrepreneurs_renew_config_id = getIntent().getStringExtra("entrepreneurs_renew_config_id");
    ...
    public void getZYBBalanceInfo() {
        showDialog();
        Observable<ZYBBDataBean> observable = Api.getInstance().service.getrenewBankInfo(
    ...
    private void applyPay() {
        ...
        Observable<PayWXDataBean> observable = Api.getInstance().service.PionXuFeiPay(UserUtils.getUserId(mContext),
                entrepreneurs_renew_config_id, money, payType, "2", payment_img, ExampleApplication.token);
        observable.subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
    ...
    private void applyPaySuccuess() {
        ...
        PioneerXuFeiActivity.mPioneerXuFeiActivity.doFinish();
        doFinish();
    }
