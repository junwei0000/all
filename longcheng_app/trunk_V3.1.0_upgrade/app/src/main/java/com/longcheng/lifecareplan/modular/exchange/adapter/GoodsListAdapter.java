...
public class GoodsListAdapter extends BaseAdapterHelper<GoodsItemBean> {
    ...
    public View getItemView(int position, View convertView, ViewGroup parent, List<GoodsItemBean> list, LayoutInflater inflater) {
        ...
        String cont = "";
        if (buy_type == 2) {
            cont = CommonUtil.getHtmlContentBig("#c18f46", mHelpItemBean.getSuper_ability()) + " 超能";
        } else if (buy_type == 3) {
            cont = CommonUtil.getHtmlContentBig("#c18f46", mHelpItemBean.getSuper_ability()) + " 超能 + "
                    + CommonUtil.getHtmlContentBig("#c18f46", mHelpItemBean.getSkb_price()) + " 寿康宝";
        } else {
            cont = CommonUtil.getHtmlContentBig("#c18f46", mHelpItemBean.getSkb_price()) + " 寿康宝";
        }
        mHolder.item_tv_skb.setText(Html.fromHtml(cont));
