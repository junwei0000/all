...
public class ActionAdapter extends BaseAdapterHelper<HomeItemBean> {
    ...
    public View getItemView(int position, View convertView, ViewGroup parent, List<HomeItemBean> list, LayoutInflater inflater) {
        ...
        GlideDownLoadImage.getInstance().loadCircleImageRoleREf2(mHelpItemBean.getImg(), mHolder.item_iv_img, 5);
        mHolder.item_tv_name1.setText(mHelpItemBean.getName1());
        String name2 = mHelpItemBean.getName2();
        if(TextUtils.isEmpty(name2)){
            name2=mHelpItemBean.getName1();
        }
        mHolder.item_tv_name2.setText(name2);
        mHolder.item_tv_num.setText(Html.fromHtml(mHelpItemBean.getAbility_price() + "  <font color=\"#666666\">生命能量</font>"));

        int width = (DensityUtil.screenWith(context) - DensityUtil.dip2px(context, 52)) / 2;
        int height = (int) (width * 0.557);
