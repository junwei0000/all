...
import com.longcheng.lifecareplan.utils.myview.address.bean.YwpAddressBean;
...
public class AddressNewPickerView extends RelativeLayout implements View.OnClickListener {
    // recyclerView 选中Item 的颜色
    private int defaultSelectedColor = Color.parseColor("#E95D1B");
    ...
    private YwpAddressBean.AddressItemBean mSelectTown;//选中 城市  bean
    private YwpAddressBean.AddressItemBean mSelectVillage;//选中 区县  bean
    ...
    public AddressNewPickerView(Context context) {
        super(context);
        init(context);
    }

    public AddressNewPickerView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public AddressNewPickerView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context) {
        mContext = context;
        // recyclerView 选中Item 的颜色
        defaultSelectedColor = context.getResources().getColor(R.color.yellow_E95D1B);
        // recyclerView 未选中Item 的颜色
...
//            method.invoke(mTabStrip, defaultSelectedColor);
    ...
    private void initData() {
        ...
        try {
            BufferedReader addressJsonStream = new BufferedReader(new InputStreamReader(mContext.getAssets().open("address_new.json")));
            String line;
    ...
    public void setDefaultTabCount(int defaultTabCount) {
...
//        }
    ...
    private void sure() {
        ...
        if (mSelectProvice != null &&
                ...
                mSelectDistrict != null &&
                mSelectTown != null) {
            if (defaultTabCount == 5) {
                if (mSelectVillage != null) {
                    //   回调接口
                    if (mOnAddressPickerSureListener != null) {
                        String address = mSelectProvice.getN() + "" + mSelectCity.getN() + "" + mSelectDistrict.getN() + "" + mSelectTown.getN() + "" + mSelectVillage.getN() + " ";
                        mOnAddressPickerSureListener.onSureClick(address, mSelectVillage.getSm(),
                                mSelectProvice.getI(), mSelectCity.getI(), mSelectDistrict.getI(), mSelectTown.getI(), mSelectVillage.getI());
                    }
                } else {
                ...
                //   回调接口
                if (mOnAddressPickerSureListener != null) {
                    String address = mSelectProvice.getN() + "" + mSelectCity.getN() + "" + mSelectDistrict.getN() + "" + mSelectTown.getN() + "";
                    mOnAddressPickerSureListener.onSureClick(address, mSelectTown.getSm(),
                            mSelectProvice.getI(), mSelectCity.getI(), mSelectDistrict.getI(), mSelectTown.getI(), "");
                }
            }
...
//        mYwpAddressBean = null;
        ...
        public void onTabSelected(TabLayout.Tab tab) {
            ...
            switch (tab.getPosition()) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    // 点到乡的时候要判断有没有选择省份与城市与县
                    if (mSelectProvice != null && mSelectCity != null && mSelectDistrict != null) {
                        for (YwpAddressBean.AddressItemBean itemBean : mYwpAddressBean.getDistrict()) {
                            if (itemBean.getP().equals(mSelectDistrict.getI()))
                                mRvData.add(itemBean);
                        }
                    } else {
                        ToastUtils.showToast("请您先选择省份与城市");
                ...
                case 4:
                    // 点到区的时候要判断有没有选择省份与城市
                    if (mSelectProvice != null && mSelectCity != null && mSelectDistrict != null && mSelectTown != null) {
                        for (YwpAddressBean.AddressItemBean itemBean : mYwpAddressBean.getDistrict()) {
                            if (itemBean.getP().equals(mSelectTown.getI()))
                                mRvData.add(itemBean);
                        }
                    } else {
                        ToastUtils.showToast("请您先选择省份与城市");
        ...
        public void onBindViewHolder(final ViewHolder holder, final int position) {
            ...
            switch (tabSelectPosition) {
                ...
                case 3:
                    if (mRvData.get(position) != null &&
                            mSelectTown != null &&
                            mRvData.get(position).getI().equals(mSelectTown.getI())) {
                        holder.mContent.setTextColor(defaultSelectedColor);
                    }
                    break;
                case 4:
                    if (mRvData.get(position) != null &&
                            mSelectVillage != null &&
                            mRvData.get(position).getI().equals(mSelectVillage.getI())) {
                        holder.mContent.setTextColor(defaultSelectedColor);
                    }
                    break;
            }
                ...
                public void onClick(View v) {
                    ...
                    switch (tabSelectPosition) {
                        ...
                        case 2:
                            ...
                            mSelectTownPosition = 0;
                            mSelectVillagePosition = 0;
                            mTabLayout.getTabAt(3).setText(defaultText);
                        ...
                        case 3:
                            ...
                            mSelectVillage = null;
                            mSelectVillagePosition = 0;
                            mTabLayout.getTabAt(4).setText(defaultText);
                            mTabLayout.getTabAt(3).setText(mSelectTown.getN());
                            mTabLayout.getTabAt(4).select();
                            mSelectTownPosition = position;
                            break;
                        case 4:
                            mSelectVillage = mRvData.get(position);
                            // 没了，选完了，这个时候可以点确定了
                            mTabLayout.getTabAt(4).setText(mSelectVillage.getN());
                            mSelectVillagePosition = position;
    ...
    public interface OnAddressPickerSureListener {
        void onSureClick(String address, String name, String provinceCode, String cityCode, String districtCode, String townCode, String villageCode);
    }
