...
public class FaceRecognitionUtils {
        ...
        HttpRequestManager.getInstance().verify(activity, ExampleApplication.VERIFY_URL, sign, ExampleApplication.SIGN_VERSION, token, data, new HttpRequestCallBack() {
            ...
            public void onSuccess(String responseBody) {
                ...
                try {
                    ...
                    savaInfoToSDNew(activity, "FaceRecognitionUtils==" + responseBody);
                } catch (JSONException e) {
                    e.printStackTrace();
                    ToastUtils.showToast("人脸认证未通过");
                    savaInfoToSDNew(activity, "JSONException==" + e.toString());
                }
            ...
            public void onFailure(int statusCode, byte[] responseBody) {
                ...
                LoadingDialogAnim.dismiss(activity);
                ToastUtils.showToast("人脸认证未通过");
                savaInfoToSDNew(activity, "onFailure==" + new String(responseBody));
            }
        });
    }
    ...
    public String savaInfoToSDNew(Context context, String cont) {
        String fileName = null;
        // 判断SD卡是否存在
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            File dir = new File(Environment.getExternalStorageDirectory().toString() + File.separator + "longcheng_log" + File.separator);
            if (!dir.exists()) {
                dir.mkdir();
            }
        ...
        // 写入手机内存
            ...
            try {
                ...
                phone_outStream.write(cont.getBytes());
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
