...
public class FinishAdapter extends BaseAdapterHelper<HeloneedBean.UserBean> {
    private final ImageLoader asyncImageLoader;
    FinishAdapter.ViewHolder mHolder = null;
    Activity context;
    String skip_source;
    ...
    public View getItemView(int position, View convertView, ViewGroup parent, List<HeloneedBean.UserBean> list, LayoutInflater inflater) {
        if (convertView == null) {
            convertView = inflater.inflate(R.layout.fragment_finish_item, parent, false);
            mHolder = new FinishAdapter.ViewHolder(convertView);
            convertView.setTag(mHolder);
        } else {
            mHolder = (FinishAdapter.ViewHolder) convertView.getTag();
        }
        ...
        mHolder.item_pb_lifeenergynum.setReachedBarColor(context.getResources().getColor(R.color.progressbarbg));
        ColorChangeByTime.getInstance().changeDrawableToClolor(context, mHolder.item_pb_numne, R.color.progressbarbg);
        return convertView;
    }

    public FinishAdapter(Activity context, List<HeloneedBean.UserBean> list, String skip_source) {
        super(context, list);
