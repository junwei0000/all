...
public class MinePresenterImp<T> extends MineContract.Present<MineContract.View> {
     ...
     * 领取节日绿卷
     *
     * @param life_basic_festival_config_id
     */
    public void recieiveFestivalCard(String user_id, String life_basic_festival_config_id) {
        mView.showDialog();
        Observable<EditDataBean> observable = Api.getInstance().service.recieiveFestivalCard(user_id, life_basic_festival_config_id, ExampleApplication.token);
        observable.subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
