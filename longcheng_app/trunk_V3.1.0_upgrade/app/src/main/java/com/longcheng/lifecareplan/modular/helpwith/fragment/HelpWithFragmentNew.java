...
public class HelpWithFragmentNew extends BaseFragmentMVP<HelpWithContract.View, HelpWithPresenterImp<HelpWithContract.View>> implements HelpWithContract.View {
     ...
     * 是否有通讯录 0:无。1:是;
     */
    public static int isHaveAddress;
    /**
     * 发福包基数;
     */
    public static int blessBagVal;
    ...
    public static List<String> zangfus;
    ...
    private void showCont() {
        ...
        mList.add(new HelpWithInfo("救急", "", R.color.white, R.mipmap.wisheach_emergency_icon, lifeUrlWorld));
//        mList.add(new HelpWithInfo("救济", "", R.color.white, R.mipmap.wisheach_relief_icon, lifeUrlWorld));
        HelpWithTopAdapter mHelpWithTopAdapter = new HelpWithTopAdapter(mActivity, mList, solarTermsEnsImg, noUserCount);
        ...
        mBottomList.add(new HelpWithInfo("进入公社", R.mipmap.in_commue));
    ...
    public void getHelpIndexSuccess(HelpIndexDataBean mHomeDataBean) {
        ...
        if (status.equals("200")) {
            ...
            isHaveAddress = mHelpIndexAfterBean.getIsHaveAddress();
            blessBagVal = mHelpIndexAfterBean.getBlessBagVal();
            noUserCount = mHelpIndexAfterBean.getNoUserCount();
            showCont();
            ...
            if (solarTermsEnsImg != null && solarTermsEnsImg.size() >= 0) {
                ...
                GlideDownLoadImage.getInstance().loadCircleImageHelpIndex(mActivity, solarTermsEnsImg.get(solarTermsEnsImg.size() - 1), iv_tohelpimg);
            }
            activity_Layers = mHelpIndexAfterBean.getActivityLayers();
            showFuBaoDialog(mActivity);
        }
    }

    ArrayList<HelpIndexItemBean> activity_Layers;

    public void showFuBaoDialog(Activity mActivity) {
        //非首页隐藏首页弹层
        if (BottomMenuActivity.position != BottomMenuActivity.tab_position_help) {
            dismiss();
            return;
        }
    ...
    public void dismiss() {
        if (mJieQiUtils != null) {
            mJieQiUtils.setActivitiesDismiss();
        }
    }
