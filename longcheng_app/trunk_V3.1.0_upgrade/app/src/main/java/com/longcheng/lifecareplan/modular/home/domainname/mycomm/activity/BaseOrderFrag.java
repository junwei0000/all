...
public abstract class BaseOrderFrag extends BaseListFrag<MyOrderContract.View, MyOrderPresenterImp<MyOrderContract.View>> implements MyOrderContract.View {
    ...
    List<HotListDataBean.HotItemInfo> mAllList = new ArrayList<>();
    private SearchListAdapter mAdapter;


    public static final int INDEX_COMM = 0;
    public static final int INDEX_DADUI = 1;
    ...
    private void getList(int page) {
        mPresent.getOrderList(user_id, (getType() + DomainMenuActivity.level_commue), page, pageSize);
    }
    ...
    @Override
    public void ListSuccess(HotListDataBean responseBean, int back_page) {
        ListUtils.getInstance().RefreshCompleteL(dateListview);
        String status_ = responseBean.getStatus();
        if (status_.equals("400")) {
            ToastUtils.showToast(responseBean.getMsg());
        } else if (status_.equals("200")) {
            ArrayList<HotListDataBean.HotItemInfo> mList = responseBean.getData();
            ...
            if (mAdapter == null) {
                mAdapter = new SearchListAdapter(getActivity(), mList, (getType() + DomainMenuActivity.level_commue), null);
