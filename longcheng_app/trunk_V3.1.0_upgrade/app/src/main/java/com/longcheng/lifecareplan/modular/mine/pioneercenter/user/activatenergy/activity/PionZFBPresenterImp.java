...
public class PionZFBPresenterImp<T> extends PionZFBContract.Presenter<PionZFBContract.View> {
    ...
    public void getRechargeHistory() {
        mView.showDialog();
        Observable<PionZFBHistoryDataBean> observable = Api.getInstance().service.getRechargeHistory(UserUtils.getUserId(mContext), ExampleApplication.token);
        observable.subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Consumer<PionZFBHistoryDataBean>() {
                    @Override
                    public void accept(PionZFBHistoryDataBean responseBean) throws Exception {
                        mView.dismissDialog();
                        if (!UserLoginBack403Utils.getInstance().login499Or500(responseBean.getStatus())) {
                            mView.historySuccess(responseBean);
                        }
                    }
                }, new Consumer<Throwable>() {
                    @Override
    ...
    public void getRechargeList(String keyword) {
        mView.showDialog();
        Observable<PionZFBSelectDataBean> observable = Api.getInstance().service.getRechargeList(UserUtils.getUserId(mContext),keyword, ExampleApplication.token);
        observable.subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
