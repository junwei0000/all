    ...
    public void getData(String userId, int page, int pageSize, int source_type, String month, int type) {
        ...
        if (type == BasicssActivity.TYPE_BILL) {
            ...
            observable = Api.getInstance().service.getSleepEngryRecordList(userId, page, pageSize,
                    source_type, month, ExampleApplication.token);
        } else if (type == BasicssActivity.TYPE_PioneerUserFQB) {
            observable = Api.getInstance().service.userFuqibaoList(userId, page, pageSize,
                    source_type, month, ExampleApplication.token);
        } else if (type == BasicssActivity.TYPE_PioneerUserJQB) {
            observable = Api.getInstance().service.userJieqibaoList(userId, page, pageSize,
                    source_type, month, ExampleApplication.token);
        } else if (type == BasicssActivity.TYPE_PioneerUserZYB) {
            observable = Api.getInstance().service.userUzhufubaoList(userId, page, pageSize,
                    source_type, month, ExampleApplication.token);
        } else if (type == BasicssActivity.TYPE_PioneerZYB) {
            observable = Api.getInstance().service.entrepreneursZhufubaoList(userId, page, pageSize,
                    source_type, month, ExampleApplication.token);
        } else if (type == BasicssActivity.TYPE_PioneerCAILI) {
            observable = Api.getInstance().service.entrepreneursRebateList(userId, page, pageSize,
                    source_type, month, ExampleApplication.token);
        } else if (type == BasicssActivity.TYPE_PioneerFQB) {
            observable = Api.getInstance().service.entrepreneursFuqibaoList(userId, page, pageSize,
                    source_type, month, ExampleApplication.token);
        } else if (type == BasicssActivity.TYPE_PioneerUserFQBEDU) {
            observable = Api.getInstance().service.entrepreneuserFuqibaoLimitList(userId, page, pageSize,
                    source_type, month, ExampleApplication.token);
        }
