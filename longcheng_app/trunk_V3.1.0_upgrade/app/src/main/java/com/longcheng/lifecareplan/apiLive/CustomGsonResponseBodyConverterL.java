 ...
 * @time 2017/3/7 10:30
 */
final class CustomGsonResponseBodyConverterL<T> implements Converter<ResponseBody, T> {
    private final Gson gson;
    private final TypeAdapter<T> adapter;
    private final JsonParser jsonParser;

    CustomGsonResponseBodyConverterL(Gson gson, TypeAdapter<T> adapter) {
        this.gson = gson;
    ...
    public T convert(ResponseBody value) throws IOException {
        ...
        JsonElement status_ = jsonElement.getAsJsonObject().get("errCode");
        String status = "";
        if (status_ != null) {
            status = status_.getAsString();
        }
        JsonElement msg_ = jsonElement.getAsJsonObject().get("errMsg");
        String msg = "";
        if (msg_ != null) {
            msg = msg_.getAsString();
        }
        Log.e("ResponseBody", "errCode=" + status + "  ;value==" + jsonElement.getAsJsonObject().toString());
