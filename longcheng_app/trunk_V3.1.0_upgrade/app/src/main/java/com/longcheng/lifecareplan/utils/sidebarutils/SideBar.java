package com.longcheng.lifecareplan.utils.sidebarutils;
...
public class SideBar extends View {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        cellHeight = getMeasuredHeight() / (indexes.size() + 5);
        topOffset = (getMeasuredHeight() - (indexes.size() * cellHeight)) / 2;//获取父控件的高度减去字母总长度/2；目的是让字母在屏幕中间
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < indexes.size(); i++) {
            ...
            float y = (cellHeight + textHeight) * 0.5f + cellHeight * i + topOffset;
