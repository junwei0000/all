...
import com.longcheng.lifecareplan.modular.mine.pioneercenter.financialmanage.tutorExit.adapter.PionTutorExitListAdapter;
...
public abstract class PionTutorExitBaseOrderFrag extends BaseListFrag<PionTutorExitContract.View, PionTutorExitPresenterImp<PionTutorExitContract.View>> implements PionTutorExitContract.View {
    ...
    List<PionTutorExitItemBean> mAllList = new ArrayList<>();
    private PionTutorExitListAdapter mAdapter;
    ...
    @Override
    protected PionTutorExitPresenterImp<PionTutorExitContract.View> createPresent() {
        return new PionTutorExitPresenterImp<>(this);
    }
    ...
    @Override
    public void ListSuccess(PionTutorExitListDataBean responseBean, int back_page) {
        ListUtils.getInstance().RefreshCompleteL(dateListview);
        String status_ = responseBean.getStatus();
        if (status_.equals("400")) {
            ToastUtils.showToast(responseBean.getMsg());
        } else if (status_.equals("200")) {
            List<PionTutorExitItemBean> mList = responseBean.getData();
            ...
            if (mAdapter == null) {
                mAdapter = new PionTutorExitListAdapter(getActivity(), mList, mHandler);
    ...
    public void editSuccess(ResponseBean responseBean) {
        ...
        if (status.equals("400")) {
            ...
            ToastUtils.showToast(responseBean.getMsg());
            ((PionTutorExitListActivity) getActivity()).reLoadList();
        }
    }
    ...
    Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            entrepreneurs_settlement_order_id = (String) msg.obj;
            switch (msg.what) {
                case SUREPAY:
                    setDialog( SUREPAY);
                    break;
    ...
    String entrepreneurs_settlement_order_id;
        ...
        if (selectDialog == null) {
            ...
            tv_sure.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    selectDialog.dismiss();
                    if (type == SUREPAY) {
                        mPresent.customerConfirmPayment(user_id, entrepreneurs_settlement_order_id);
                    }
                }
            });
        } else {
