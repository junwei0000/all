...
public class HelpWithTopAdapter extends BaseAdapterHelper<HelpWithInfo> {
    ...
    MyAnimationUtils mAnimationUtils;
    int noUserCount;

    public HelpWithTopAdapter(Context context, List<HelpWithInfo> list, List<String> solarTermsEnsImg, int noUserCount) {
        super(context, list);
        this.context = context;
        this.solarTermsEnsImg = solarTermsEnsImg;
        this.noUserCount = noUserCount;
        mAnimationUtils = new MyAnimationUtils();
    }
    ...
    public View getItemView(int position, View convertView, ViewGroup parent, List<HelpWithInfo> list, LayoutInflater inflater) {
        ...
        if (position == 2 && noUserCount > 0) {
            if (noUserCount > 99) {
                mHolder.item_tv_fuquannum.setText("…");
            } else {
                mHolder.item_tv_fuquannum.setText("" + noUserCount);
            }
            mHolder.item_tv_fuquannum.setVisibility(View.VISIBLE);
            mAnimationUtils.AlphaAnimationNew(mHolder.item_tv_fuquannum);
        } else {
            mHolder.item_tv_fuquannum.setVisibility(View.GONE);
        }
        ...
        mHolder.relat_help.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                LogUtils.v("TAG","NAME:"+list.get(position).getName()+"URL:"+list.get(position).getSkipurl());
                if (position == 0) {
                    //生命能量互祝
                    ...
                    //智能互祝
                    intent = new Intent(context, FuQCenterActivity.class);
                    intent.setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
//                    intent.putExtra("html_url", "" + list.get(position).getSkipurl());
                    ...
                    //救急
                    intent = new Intent(context, EmergencyListActivity.class);
                    context.startActivity(intent);
                } else if (position == 7) {
                    //救济
                    ToastUtils.showToast("功能优化升级中");
                    // context.startActivity(new Intent(context, ReliefActivity.class));
//                    intent = new Intent(context, BaoZhangActitvty.class);
//                    intent.setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
//                    intent.putExtra("html_url", "" + list.get(position).getSkipurl());
//                    context.startActivity(intent);
    ...
    private class ViewHolder {
        ...
        private TextView item_tv_fuquannum;


        public ViewHolder(View view) {
            relat_help = view.findViewById(R.id.relat_help);
            item_tv_cont = view.findViewById(R.id.item_tv_cont);
            item_tv_cont2 = view.findViewById(R.id.item_tv_cont2);
            item_iv_select = view.findViewById(R.id.item_iv_select);
            item_iv_img = view.findViewById(R.id.item_iv_img);
            relat_apply = view.findViewById(R.id.relat_apply);
            item_iv_applyselect = view.findViewById(R.id.item_iv_applyselect);
            layout_apply = view.findViewById(R.id.layout_apply);
            item_iv_apply = view.findViewById(R.id.item_iv_apply);
            item_tv_fuquannum = view.findViewById(R.id.item_tv_fuquannum);
        }
    }
}
