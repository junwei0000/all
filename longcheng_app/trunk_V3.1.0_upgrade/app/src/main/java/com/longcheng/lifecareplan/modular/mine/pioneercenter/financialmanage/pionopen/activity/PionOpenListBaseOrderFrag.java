...
import com.longcheng.lifecareplan.modular.mine.pioneercenter.financialmanage.pionopen.adapter.PionOpenListAdapter;
...
public abstract class PionOpenListBaseOrderFrag extends BaseListFrag<PionOpenListContract.View, PionOpenListPresenterImp<PionOpenListContract.View>> implements PionOpenListContract.View {
    ...
    List<PionOpenListItemBean> mAllList = new ArrayList<>();
    private PionOpenListAdapter mAdapter;
    ...
    private void getList(int page) {
        Log.e("getOrderList", "PionOpenListActivity.type=" + PionOpenListActivity.type);
        mPresent.getOrderList(user_id, PionOpenListActivity.type, getSearchType(), page, pageSize);
    }
    ...
    @Override
    protected PionOpenListPresenterImp<PionOpenListContract.View> createPresent() {
        return new PionOpenListPresenterImp<>(this);
    }
    ...
    @Override
    public void ListSuccess(PionOpenListDataBean responseBean, int back_page) {
        ListUtils.getInstance().RefreshCompleteL(dateListview);
        String status_ = responseBean.getStatus();
        if (status_.equals("400")) {
            ToastUtils.showToast(responseBean.getMsg());
        } else if (status_.equals("200")) {
            List<PionOpenListItemBean> mList = responseBean.getData();
            ...
            if (mAdapter == null) {
                mAdapter = new PionOpenListAdapter(getActivity(), mList, mHandler);
    ...
    public void editSuccess(ResponseBean responseBean) {
        ...
        if (status.equals("400")) {
            ...
            ToastUtils.showToast(responseBean.getMsg());
            ((PionOpenListActivity) getActivity()).reLoadList();
        }
    }
    ...
    Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            entrepreneurs_order_id = (String) msg.obj;
            switch (msg.what) {
                case SUREPAY:
                    setDialog(SUREPAY);
                    break;
        ...
        if (selectDialog == null) {
            ...
            tv_sure.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    selectDialog.dismiss();
                    if (type == SUREPAY) {
                        mPresent.customerConfirmPayment(user_id, entrepreneurs_order_id);
                    }
                }
            });
        } else {
