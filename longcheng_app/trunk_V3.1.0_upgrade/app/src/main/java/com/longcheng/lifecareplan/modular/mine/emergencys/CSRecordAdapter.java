...
import com.longcheng.lifecareplan.modular.mine.emergencys.bean.CSRecordList;
...
public class CSRecordAdapter extends BaseAdapterHelper<CSRecordList> {
    ...
    public View getItemView(int position, View convertView, ViewGroup parent, List<CSRecordList> list, LayoutInflater inflater) {
        ...
        mHolder.tv_copy.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                int position = (int) v.getTag();
                String content = "打款银行信息\n"
                        + "用户ID：" + list.get(position).getUser_id() + "\n"
                        + "姓名：" + list.get(position).getCardholder_name() + "\n"
                        + "开户行：" + list.get(position).getBank_name() + "\n"
                        + "开户支行：" + list.get(position).getBank_full_name() + "\n"
                        + "现金：" + list.get(position).getUser_money() + "元\n"
                        + "卡号：" + list.get(position).getBank_no();
