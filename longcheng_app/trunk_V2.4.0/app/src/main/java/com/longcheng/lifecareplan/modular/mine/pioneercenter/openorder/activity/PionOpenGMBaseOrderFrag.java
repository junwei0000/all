package com.longcheng.lifecareplan.modular.mine.pioneercenter.openorder.activity;
...
import com.longcheng.lifecareplan.modular.mine.pioneercenter.openorder.adapter.PionOpenGMListAdapter;
import com.longcheng.lifecareplan.modular.mine.pioneercenter.openorder.bean.PionOpenGMItemBean;
...
public abstract class PionOpenGMBaseOrderFrag extends BaseListFrag<PionOpenGMContract.View, PionOpenGMPresenterImp<PionOpenGMContract.View>> implements PionOpenGMContract.View {
    ...
    List<PionOpenGMItemBean> mAllList = new ArrayList<>();
    private PionOpenGMListAdapter mAdapter;
    ...
    @Override
    protected PionOpenGMPresenterImp<PionOpenGMContract.View> createPresent() {
        return new PionOpenGMPresenterImp<>(this);
    }
    ...
    @Override
    public void ListSuccess(PionOpenGMListDataBean responseBean, int back_page) {
        ListUtils.getInstance().RefreshCompleteL(dateListview);
        String status_ = responseBean.getStatus();
        if (status_.equals("400")) {
            ToastUtils.showToast(responseBean.getMsg());
        } else if (status_.equals("200")) {
            List<PionOpenGMItemBean> mList = responseBean.getData();
            ...
            if (mAdapter == null) {
                mAdapter = new PionOpenGMListAdapter(getActivity(), mList, mHandler);
    ...
    public void editSuccess(ResponseBean responseBean) {
        ...
        if (status.equals("400")) {
            ...
            ToastUtils.showToast(responseBean.getMsg());
            ((PionOpenGMListActivity) getActivity()).reLoadList();
        }
    }
    ...
    Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            String entrepreneurs_apply_money_id = (String) msg.obj;
            switch (msg.what) {
                case SUREPAY:
                    setDialog(entrepreneurs_apply_money_id, SUREPAY);
                    break;
            }
        }
    };
    ...
    public void setDialog(String entrepreneurs_apply_money_id, int type) {
        if (selectDialog == null) {
            ...
            tv_sure.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    selectDialog.dismiss();
                    if (type == SUREPAY) {
                        mPresent.customerConfirmPayment(user_id, entrepreneurs_apply_money_id);
                    }
                }
            });
        } else {
