...
import com.longcheng.lifecareplan.modular.mine.pioneercenter.order.adapter.PionRewardListAdapter;
...
public abstract class PionBaseOrderFrag extends BaseListFrag<PionRewardContract.View, PionRewardPresenterImp<PionRewardContract.View>> implements PionRewardContract.View {
    ...
    List<PionRewardItemBean> mAllList = new ArrayList<>();
    private PionRewardListAdapter mAdapter;
    ...
    @Override
    protected PionRewardPresenterImp<PionRewardContract.View> createPresent() {
        return new PionRewardPresenterImp<>(this);
    }
    ...
    @Override
    public void ListSuccess(PionRewardListDataBean responseBean, int back_page) {
        ListUtils.getInstance().RefreshCompleteL(dateListview);
        String status_ = responseBean.getStatus();
        if (status_.equals("400")) {
            ToastUtils.showToast(responseBean.getMsg());
        } else if (status_.equals("200")) {
            List<PionRewardItemBean> mList = responseBean.getData();
            ...
            if (mAdapter == null) {
                mAdapter = new PionRewardListAdapter(getActivity(), mList, mHandler);
    ...
    public void editSuccess(ResponseBean responseBean) {
        ...
        if (status.equals("400")) {
            ...
            ToastUtils.showToast(responseBean.getMsg());
            ((PionRewardListActivity) getActivity()).reLoadList();
        }
    }
    ...
    Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            String entrepreneurs_fuqibao_sell_order_id = (String) msg.obj;
            switch (msg.what) {
                case SUREPAY:
                    setDialog(entrepreneurs_fuqibao_sell_order_id, SUREPAY);
                    break;
                case CANCEL:
                    setDialog(entrepreneurs_fuqibao_sell_order_id, CANCEL);
                    break;
            }
        }
    };
    ...
    public void setDialog(String entrepreneurs_fuqibao_sell_order_id, int type) {
        if (selectDialog == null) {
            ...
            tv_sure.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    selectDialog.dismiss();
                    if (type == SUREPAY) {
                        mPresent.customerConfirmPayment(user_id, entrepreneurs_fuqibao_sell_order_id);
                    } else {
                        mPresent.PioncustomerBoHui(user_id, entrepreneurs_fuqibao_sell_order_id);
                    }
                }
            });
        } else {
