 ...
 * 提现--已收款上传凭证
 */
public class TiXianConfirmActivity extends BaseListActivity<EnergyCenterContract.View, EnergyCenterPresenterImp<EnergyCenterContract.View>> implements EnergyCenterContract.View {
    ...
    @BindView(R.id.tv_montitle)
    TextView tv_montitle;
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_givemoney:
                if (!TextUtils.isEmpty(bless_payment_photo)) {
                    if (!requestStatus) {
                        requestStatus = true;
                        mPresent.userConfirmCash(user_zhufubao_order_id, bless_payment_channel, bless_payment_photo);
                    }
                } else {
    ...
    public void setListener() {
        pageTopTvName.setText("上传收款凭证");
        btnGivemoney.setText("确定已收款");
        tv_montitle.setText("请选择您向祝福师提现的收款方式");
    ...
    public void initDataAfter() {
        ...
        iv_code.setLayoutParams(new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.MATCH_PARENT, hei));
    ...
    private void zfbSelectPayType() {
        ...
        if (bless_payment_channel == 1) {
            ...
            tvTishiTitle.setText("上传支付宝收款凭证");
            ...
            tvTishiTitle.setText("上传银行卡收款凭证");
    ...
    public void RefuseSuccess(EditListDataBean responseBean) {
        ...
        intent.setAction("TiXianRecordActivity");
