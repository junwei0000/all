...
import com.longcheng.lifecareplan.modular.mine.pioneercenter.user.recovercash.daifu.imguide.IMGuideUtils;
 ...
 * 提现记录
 */
public class PionTiXianRecordActivity extends BaseActivity implements ViewPager.OnPageChangeListener {
    ...
    @BindView(R.id.userorder_tv_tishi)
    TextView userorder_tv_tishi;
    ...
    LinearLayout pageTop_layout_num;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.pagetop_layout_rigth:
                IMTool.getINSTANCE().startConversationList(mContext);
                break;
    ...
    public void setListener() {
        int hei = DensityUtil.dip2px(mContext, 20);
        pagetop_iv_rigth.setBackgroundResource(R.mipmap.my_liantian_icon);
        pagetop_iv_rigth.setLayoutParams(new LinearLayout.LayoutParams(hei, hei));
        int top = DensityUtil.dip2px(mContext, 5);
        pageTop_layout_num.setPadding(0, top, top, 0);
    ...
    IMGuideUtils imGuideUtils;

    private void showImGuide() {
        imGuideUtils = new IMGuideUtils(mActivity);
        if (!imGuideUtils.getCommGuide())
            imGuideUtils.showGuideDialog("user");
    }
    ...
    private void selectPage() {
        ...
        userorder_tv_tishi.setVisibility(View.GONE);
        if (position == 0) {
            userorderTvAll.setTextColor(getResources().getColor(R.color.blue));
        } else if (position == 1) {
            userorder_tv_tishi.setVisibility(View.VISIBLE);
    ...
    protected void onResume() {
        super.onResume();
        IMTool.getINSTANCE().setUnReadNum(top_tv_num);
        if (editOrderStatus) {
