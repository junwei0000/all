...
public class DaiFusAdapter extends BaseAdapterHelper<DaiFuItemBean> {
    ViewHolder mHolder = null;
    Context context;

    Handler mHandler;

    public DaiFusAdapter(Context context, List<DaiFuItemBean> list, Handler mHandler) {
        super(context, list);
        ...
        mHolder.item_tv_rigth.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (user_status == 0 && bless_user_status == 0) {// 已打款
                    message.what = DaiFuActivity.blessPaymentConfirm;
                    message.obj = user_zhufubao_order_id;
                    message.arg1 = mDaiFuItemBean.getPosition();
                    mHandler.sendMessage(message);
                } else if (user_status == -1) {
                    message.what = DaiFuActivity.ConfirmTiXian;
                    message.obj = user_zhufubao_order_id;
                    message.arg1 = mDaiFuItemBean.getPosition();
                    mHandler.sendMessage(message);
                }
