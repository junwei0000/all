...
public class UpdateInfoUtils {
	...
	private void showDilag() {
		try {
			if (mDialog == null) {
				mDialog = CommonUtils.getInstance().createLoadingDialog(mContext);
			} else {
				mDialog.show();
			}
		} catch (Exception e) {
	...
	public void UpdateInfo(String typekey, String typevalue) {
		if (typekey.equals("hobby")) {
			...
			sex = Integer.parseInt(typevalue);
	...
	private void processUpdateInfo(final String typekey,final String typevalue) {
		if (!ConfigUtils.getInstance().isNetWorkAvaiable(mContext)) {
			CommonUtils.getInstance().initToast(mContext, mContext.getString(R.string.net_tishi));
			return;
		}
		showDilag();
		...
		new UserAccountUpdateBusiness(mContext, "info", mhashmap, new GetAccountUpdateCallback() {
